...
(function($) {
    ...
    var qLoptions = {
        ...
        onLoadComplete: function () {
            if (qLoptions.completeAnimation == "grow") {
				...
				$(qLoverlay).delay(800).fadeOut(500, function () {
                    $(qLoverlay).remove();
                    qLoptions.onComplete();
                });
            }
    ...
    var createOverlayLoader = function () {
        ...
        qLbar = $("<div id='qLbar'></div>").css({
			...
			border: qLoptions.barBorder,
            width: "0%",
            position: "absolute",
            top: "50%"
        }).appendTo(qLoverlay);
		qLlogo = $("<div id='qLlogo'></div>").css({
			height:"200px",
			width:"200px",
			opacity: 0,
			...
			backgroundImage:"url('http://parameter.cc/images/logo2.png')",
			backgroundRepeat:"no-repeat",
		}).animate({
                opacity: 1,
                top: [290, 'easeOutBounce']
            }, 800)
            .delay(5000)
...
var GravityC = {
	...
	init: function() {
		...
		$("#air").css({
			...
			var yposBlock = $(block).position()['top'] - $("#air").position()['top'];
			var fallDist = ($("#air").outerHeight() - yposBlock) - $(block).outerHeight() -50;
